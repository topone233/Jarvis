...
已经能用的：选数据目录 → 配模型 → 建会话 → 发消息 → 流式回复 → 刷新续传 → 终止 / 重新生成 / 点赞点踩 → 会话增删与切换 → 模型配置与会话期增删改 → 在输入框里换模型、拨思考强度 → 记忆页（查看 / 编辑 / 删除 / 恢复 / 永久删除）→ 知识库 → 技能（导入通用 Agent Skills 格式的 skill 文件夹、开关、删除；对话里 `/技能名` 直接触发，或由模型自行调用，技能自带脚本可执行）。
