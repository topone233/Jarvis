...
已经能用的：选数据目录 → 配模型 → 建会话 → 发消息 → 流式回复 → 刷新续传 → 终止 / 重新生成 / 点赞点踩 → 会话增删与切换 → 模型配置与会话期增删改 → 在输入框里换模型、拨思考强度。

**还没有的**（后端接口在，界面没做）：右侧对话目录、回收站页面、项目页、知识库页、记忆页、输入框的 `+` 附件菜单、Slash 命令、Mermaid / SVG / HTML 预览卡片。
...
- **消息不单独删除。** 删除的单位是会话（走回收站），所以一轮问答不会只剩一半，重新生成也就可以假定最新一条回复一定在。
- **删除是软删除。** `DELETE /api/trash/{id}` 不擦除物理行——`assistant_runs` 外键指向消息，run 日志就是审计轨迹。真要"从磁盘上消失"是另一个特性，现在没有。
- **数据目录不自动创建**，见上文"第一次打开"。
